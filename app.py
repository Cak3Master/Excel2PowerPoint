...
def find_actual_data_range(worksheet):
    """Find the actual data range, excluding empty rows/columns at the start"""
...
def calculate_optimal_column_widths(worksheet, start_row, start_col, data_rows, data_cols, available_width):
...
def calculate_table_dimensions(worksheet, start_row, start_col, max_rows, max_cols):
    ...
    col_widths = calculate_optimal_column_widths(worksheet, start_row, start_col, max_rows, max_cols, available_width)
...
def create_grouped_shape_table(slide, worksheet, start_row, start_col, data_rows, data_cols, left, top, table_width, font_size, col_widths, row_height):
    ...
    for row_idx in range(data_rows):
        for col_idx in range(data_cols):
            try:
                ...
                excel_cell = worksheet.cell(row=start_row + row_idx, column=start_col + col_idx)
...
def convert_excel_to_ppt(excel_file_path):
    ...
    try:
        ...
        for sheet_name in workbook.sheetnames:
            ...
            # Find actual data range (trim whitespace)
            ...
            table_width, available_height, font_size, col_widths, row_height = calculate_table_dimensions(worksheet, start_row, start_col, data_rows, data_cols)
            ...
            table_shape = create_grouped_shape_table(slide, worksheet, start_row, start_col, data_rows, data_cols, left, top, table_width, font_size, col_widths, row_height)
